...
export default function ViewSecret() {
  ...
  const checkStatus = async (decryptToken: string) => {
    try {
  ...
  const handleRetrieve = async () => {
    ...
    try {
      const result = await retrieveSecret(token);
  ...
  if (state.type === 'not_found') {
    return (
      <div className="view-secret">
        ...
        <p>
          This secret doesn't exist. It may have been deleted, already retrieved, or
  ...
  if (state.type === 'already_retrieved') {
    return (
      <div className="view-secret">
        ...
        <p>
          This secret has already been retrieved and is no longer available.
          Secrets can only be accessed once.
        </p>
      </div>
    );
  }
    ...
    return (
      <div className="view-secret">
        ...
        <p className="unlock-date">
          Unlocks on: {state.unlockAt.toLocaleDateString()} at{' '}
          {state.unlockAt.toLocaleTimeString()}
        </p>
      </div>
    );
  }
    ...
    return (
      <div className="view-secret">
        ...
        <div className="warning">
          <strong>Warning:</strong> This secret can only be retrieved once. After you
          click the button below, it will be permanently deleted.
        </div>
  ...
  if (state.type === 'decrypted') {
    return (
      <div className="view-secret">
        ...
        <div className="info">
          This secret has been deleted from our servers. Save this message if you
          need to keep it.
        </div>
      </div>
    );
